...
from SaveDataBaseSettingFile import SaveDataBaseSettingFile
from SettingExcelPathScreen import SettingExcelPathScreen

class SaveLogDBSettingScreen():

    def draw_GUI_screen(self):

        self.window = Tk()
        self.window.title("Save Log DB 설정 화면")  # 메인 화면 타이틀 제목 설정
        ...
        # DB 설명 라벨
        #str_discription1 = "** 해당 화면은 DB 화면을 저장할 데이터 베이스의 정보를 기록 하는 화면 입니다 **"
        #str_discription2 =  "(데이터를 EXCEL로만 저장하실경우 DB정보를 입력 하실 필요 없습니다)"

        #IPLabel = Label(self.window, text=str_discription1)  # 데이터 베이스 IP 라벨
        #IPLabel.place(x=120, y=400)  # IPLabel의 위치

        #IPLabel = Label(self.window, text=str_discription2)  # 데이터 베이스 IP 라벨
        #IPLabel.place(x=120, y=430)  # IPLabel의 위치
    ...
    def next_Click(self):
        ...
        save_DB_setting = SaveDataBaseSettingFile()
        save_DB_setting.wrtite_txt_file(textList)

        #GUI창 종료
        self.window.destroy()

        # 엑셀 경로 설정 화면 호출
        setting_excel_path_Screen = SettingExcelPathScreen()
        setting_excel_path_Screen.draw_GUI_screen()